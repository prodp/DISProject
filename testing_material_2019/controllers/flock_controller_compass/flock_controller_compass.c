...
#define KP              0.23   // Proportional gain for angular velocity
#define ALPHA           3.01       // Proportional constant for heading
#define BETA            1.76      // Weight of proximal control
#define DELTA           1.57     // Proportional constant for cohesion
#define ZETA            1.45     // Gain for cohesion non-linearity
#define IOTA            1.33     // Gain for proximal non-linearity
#define MIGRATION_BIAS  0.667     // Migration bias
    ...
    while(1){
        ...
        for(int i = 0; i<FLOCK_SIZE;++i){
            if((tick-last_heard_of[i])>TIMEOUT_STEPS || !initialized[i]){
                ...
                if (relative_pos[i][2] > M_PI) relative_pos[i][2] -= 2.0*M_PI;
                if (relative_pos[i][2] < -M_PI) relative_pos[i][2] += 2.0*M_PI;
            }
        }
