...
#define MIN_SENS        50     // Minimum sensibility value
#define MAX_SENS        500    // Maximum sensibility value
#define MAX_SPEED       200   // Maximum speed
#define MAX_RADIO_RANGE 0.15     // Maximum radio range

#define FLOCK_SIZE      3       // Size of flock
#define TIME_STEP       100      // [ms] Length of time step
...
#define EPSILON         1e-6
#define DESIRED_CM      0.075 // Desired distance to center of mass [cm]
...
void initialize_positions(void) {
    ...
    while (wb_receiver_get_queue_length(receiver) > 0) {
        ...
        other_theta = atof(inbuffer+2);

        message_rssi = wb_receiver_get_signal_strength(receiver);
        range = sqrt(1/message_rssi);

        if((other_flock == flock) && (initialized[other_id] == 0) && (range < MAX_RADIO_RANGE)) {
            message_direction = wb_receiver_get_emitter_direction(receiver);

            //Get relative position and angle of message
            theta = atan2(message_direction[0],message_direction[2]);
            float other_pos[2] = {range*cos(theta), -1*range*sin(theta)};

            //Update position and speed
            for(int i=0; i<2; i++) {
                relative_pos[other_id][i] = other_pos[i];
            }
            relative_pos[other_id][2] = other_theta;
...
void compute_speed(int *msl, int *msr, double weights[DATASIZE]){
  ...
  */
  ...
  if(dot > 0){
      if(GAMMA != 0){
          if(dot_c_h<0 && p[0] >= 0)
            u = dot*2*PI*WHEEL_RADIUS*sqrt(1-ABS(dot_c_h));
          else
            u = dot*2*PI*WHEEL_RADIUS;
      }
      else{
          u = 2*PI*WHEEL_RADIUS;
      }
  } else if(dot_c_h<0) {
      u = ABS(dot)*2*PI*WHEEL_RADIUS*sqrt(1-ABS(dot_c_h));
  }
  float w = (phase_a)*KP*8*WHEEL_RADIUS/AXLE_LENGTH;

  *msl = (u - w*AXLE_LENGTH/2)*MAX_SPEED/(2*PI*WHEEL_RADIUS);
  *msr = (u + w*AXLE_LENGTH/2)*MAX_SPEED/(2*PI*WHEEL_RADIUS);
