  ...
  for (k = 0; k < ITS_COEFF*iterations; k++) {
    ...
    for (i = 0; i < swarmsize; i++) {
      for (j = 0; j < datasize; j++) {
        ...
        if(j>=(datasize -2))
          v[i][j] += lweight*rnd()/10*(lbest[i][j] - swarm[i][j]) + nbweight*rnd()/10*(nbbest[i][j] - swarm[i][j]);
        else
...
void initFile(int index_file, char* field_name){
    char filename[512];
    sprintf(filename, "../../../matlab/output_pso_calib_%s.m", field_name);
    files[index_file] = fopen(filename,"w");
    fprintf(files[index_file], "%s = [", field_name);
}
